...
import { TILE, choice, randi } from "./util.js?v=__BUILD__";
...
function makeFloorTile(variant) {
  ...
  for (let i = 0; i < 7 + variant * 2; i++) {
    ctx.fillStyle = choice(speckles);
    ctx.fillRect(randi(2, 28), randi(2, 28), randi(2, 4), randi(2, 3));
  }
...
function makeDoorTile(open) {
  const t = makeCanvas(TILE, TILE);
  const ctx = t.getContext("2d");
  ctx.drawImage(sprites.wallTile, 0, 0);
...
function makeThemedFloor(pal, variant) {
  return paintTile((ctx) => {
    ...
    for (let i = 0; i < 6 + variant * 2; i++) {
      ctx.fillStyle = choice(pal.speckles);
      ctx.fillRect(randi(2, 28), randi(2, 28), randi(2, 4), randi(2, 3));
    }
...
function makeWoodFloor(variant) {
  return paintTile((ctx) => {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      ctx.fillRect(0, i * 8, TILE, 8);
      ctx.fillStyle = "#6e4a23"; // groove
      ctx.fillRect(0, i * 8 + 7, TILE, 1);
    }
    ctx.fillStyle = "#8a5e2e"; // plank butt joints
    ctx.fillRect((variant * 11) % TILE, 0, 1, TILE);
  });
}
...
function makeRailSegment() {
  return paintTile((ctx) => {
    ...
    for (let y = 2; y < TILE; y += 11) ctx.fillRect(4, y, 24, 3);
    ctx.fillStyle = "#6a6a72"; // steel rails
    ctx.fillRect(9, 0, 3, TILE);
    ctx.fillRect(20, 0, 3, TILE);
    ctx.fillStyle = "#8a8a94"; // rail highlight
    ctx.fillRect(9, 0, 1, TILE);
    ctx.fillRect(20, 0, 1, TILE);
  });
}
...
function makeBarCounter() {
  const c = makeCanvas(TILE * 4, TILE * 2);
  const ctx = c.getContext("2d");
  const W = TILE * 4;
